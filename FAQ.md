...
> [!IMPORTANT]
> No database data is sent to any cloud service. Everything stays private on your local machine.

---

### Q2 — Which features read and write to the local database?
**A:** Every feature that stores records uses the local MySQL database. Here is the mapping:

| Feature Area | Database Tables Used |
| :--- | :--- |
| **User accounts & auth** | `users` |
| **Class routine / schedule** | `schedules` |
| **Notes & class materials (metadata)** | `materials` |
| **Community posts & feedback** | `posts`, `likes`, `comments`, `comment_likes` |
...
| **Documentation & Team** | `docs_sections`, `docs_settings`, `docs_team_members` |

---

## 🌥️ Media Storage (Cloudinary vs. Local)

### Q3 — What is Cloudinary used for?
**A:** Cloudinary is a cloud media storage service. Campus Buddy uses it to host **uploaded images and documents** so they are available on any device/internet connection, rather than being restricted to your local machine.

---

### Q4 — Which features upload files to Cloudinary?

| Feature | Uploaded Assets | Cloudinary Folder |
| :--- | :--- | :--- |
| **Profile Image** | User avatar photo | `profile_images/` |
...
> [!TIP]
> These features **still upload to Cloudinary** and have not been changed.

---

### Q5 — Which features use local storage instead of Cloudinary?

| Feature | Old Behaviour | New Behaviour (Local Dev) |
| :--- | :--- | :--- |
| **Class materials / PDF notes upload** | Uploaded to Cloudinary as raw files | **Saved locally** to `storage/app/public/materials/` |
...
> 1. The **AI PDF/Note Summarizer** (could not parse the file structure).
> 2. The **View / Download** buttons (downloaded as unreadable binary files).
> 
> Saving locally means the file is readable directly by the PHP parser with zero HTTP round-trip latency.

---

### Q6 — Are old materials uploaded before this change still on Cloudinary?
...
*   **Cloudinary (old uploads):** 11 files
*   **Local Disk (new uploads):** 6 files

The application handles both automatically — it detects whether the path is an HTTP URL (Cloudinary) or a relative path (local) and routes requests accordingly.

---

### Q7 — Can the AI Summarizer read files from both local and Cloudinary?
**A:** Yes, both paths are fully supported:

| File Location | Retrieval Mechanism |
| :--- | :--- |
| **Local** (`storage/app/public/...`) | Reads the file directly from disk — fastest path |
...
### Q9 — Does the AI store my conversations in the cloud?
...
| Feature | Data Location | Files Location |
| :--- | :--- | :--- |
...
> [!WARNING]
> The local-storage switch for class materials applies to your **local development environment only** and has not been pushed to the live GitHub repository.
...
**A:** In a Laravel application, there are **four primary ways** to see the exact error message, filename, and line number causing the crash:
...
## 💡 System Design, Security, and Feasibility
